...
create_gadget_parameter_set()
{
    ...
    param_declare_int(ps, "GWRecoilSCKick", OPTIONAL, 0, "If 1, check if the GW recoil kick ejects the merger remnant from its host star cluster: the whole cluster state is removed if v_kick > v_esc, with v_esc = 33.4 km/s (M_sc/1e5 Msun)^1/2 (r_h/pc)^-1/2 evaluated on the kept cluster's CURRENT mass (StarClusterMass) and size (r_h = 4/3 SC_Reff, the effective radius as evolved by StarClusterSizeEvolution; the initial radius, or the StarClusterReffRelation median, if it is not set yet after a restart from an older snapshot). Requires StarClusterOn=1.");

...
create_gadget_parameter_set()
{
    ...
    param_declare_int(ps, "BHseedSecFOFbound", OPTIONAL, 0, "Only used with SeedInSecFOFasStarCluster=1. Restricts the star particles that contribute Gamma*m_star to the BH-seeding budget of a secondary-FOF group to the ones gravitationally bound to that group. 0 = off (every member star contributes, the default). 1 = keep only stars bound to (all member stars + all DM inside Rmax). 2 = keep only stars bound to (all member stars + the DM inside min(2*R50, Rmax)); Rmax is set by the single most distant member star and follows a percolating FoF, so 2*R50 keeps the DM test on the object itself. The potential is the spherically-averaged profile about SecPotMinPos (softened, O(N log N)); the rest frame is the member stars' mass-weighted centre of mass. Only the seeding budget changes: the SecPIG catalogue and StarClusterDetails still describe ALL member stars, with the bound subset reported in the added SecBound* / BoundStarMass fields (0 when this is off). The restriction is applied at GROUP level (the summed budget and the single seed host). Per-cluster seeding (SecFOFseedsumover=0) is also supported: it picks its own hosts, so it is given a transient per-particle bound flag that restricts its host pool, and with it the seed count (a group places min(n_request, bound hosts) seeds). That flag lives only inside the seeding call, so the modes that re-scan the unseeded stars outside it remain incompatible and abort at startup: SeedSecFOFcomSampleParticle=1 (its redistribution runs later, in secondfof_seed, and would divide the group total by the bound stellar mass while summing over all unseeded stars), SeedInSecFOFMultipleSeeds=1 (extra hosts are not boundedness-tested) and SeedSeedFOFMassiveBoundStar=1 (same job, would apply twice). Note the Seeded=1 flag is still set on every seedable star of a seeded group, bound or not, matching the single-seed path: an unbound star is spent without having contributed.");
    ...
    param_declare_double(ps, "StarClusterSeedMetallicityMin", OPTIONAL, 0, "Metallicity-dependent seeding factor f(Z): lower log10(Z/Zsun) threshold (Zsun=0.0134). For log10(Z/Zsun) <= this, f=1. Z is the star BirthMetallicity. If StarClusterSeedMetallicityMin == StarClusterSeedMetallicityMax (default both 0), the metallicity dependence is disabled (f=1 always).");
    param_declare_double(ps, "StarClusterSeedMetallicityMax", OPTIONAL, 0, "Metallicity-dependent seeding factor f(Z): upper log10(Z/Zsun) threshold (Zsun=0.0134). For log10(Z/Zsun) >= this, f=0; log-linear decline from 1 to 0 between Min and Max. Only used when Max > Min.");
    param_declare_double(ps, "StarClusterFixReff", OPTIONAL, 0, "Only used in the per-cluster secFOF seeding mode (SecFOFseedsumover=0). Effective radius (in pc) assigned to every seeded star cluster. If > 0, this fixed value replaces the size-mass relation R_eff = 1.4 pc * (M_cl/1e4 Msun)^0.25 (with its 0.5 dex scatter) for all clusters. Default 0 uses the size-mass relation.");

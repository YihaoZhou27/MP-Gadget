...
create_gadget_parameter_set()
{
    ...
    param_declare_int(ps, "GWRecoilSCKick", OPTIONAL, 0, "If 1, check if the GW recoil kick ejects the merger remnant from its host star cluster: the whole cluster state is removed if v_kick > v_esc, the central escape velocity of a Bahcall & Wolf (1976) cusp rho ~ r^-7/4 truncated at the cluster radius (the cluster+BH profile of Williams et al. 2026): v_esc^2 = 10 G M_sc/r_max = 4.1756 G M_sc/R_eff, i.e. v_esc = 42.4 km/s (M_sc/1e5 Msun)^1/2 (R_eff/pc)^-1/2 (R_eff = 0.418 r_max is the projected half-mass radius of that profile; a Plummer sphere would give 28.9 km/s), evaluated on the kept cluster's CURRENT mass (StarClusterMass) and effective radius (SC_Reff as evolved by StarClusterSizeEvolution; the initial radius, or the StarClusterReffRelation median, if it is not set yet after a restart from an older snapshot). Requires StarClusterOn=1.");
    ...
    param_declare_double(ps, "StarClusterSeedMaxAgeMyr", OPTIONAL, 0, "Maximum stellar age, in Myr, at which a star particle may still contribute to a star-cluster BH seed. Default 0 (and any value <= 0): no age limit, every unseeded star counts, as before. If > 0, a star older than the limit at the seeding pass is left out of its group's star-cluster seeding budget (StarClusterMassUnseeded and the sampled/metallicity/M_cut statistics built from the same set) and may not be chosen as the host star that is converted into the BH particle. Such a star is NOT consumed: it keeps its ClusterMass, stays Seeded = 0 and is still a primary-linking particle, so the secondary-FOF groups themselves are unchanged -- it simply never feeds a seed again. Motivated by the cluster formation efficiency Gamma being evaluated once, from the gas the star formed out of: the limit bounds how stale that estimate can be when it is spent. Applies to every star-cluster seeding path (SeedInSecFOFasStarCluster, BlackHoleSeedStarCluster and BlackholeSeedSCparticle). For reference, the seeding cadence itself sets a floor on the ages that occur: with TimeBetweenSeedingSearch = 1.003 the typical age at seeding is a few Myr, so a limit below the pass interval would discard stars that simply had no earlier pass to be spent at.");
    ...
    param_declare_int(ps, "SCEvolutionStellar", OPTIONAL, 0, "If 1, the star clusters attached to BH particles lose mass by stellar evolution: StarClusterMass is the birth mass times one minus the cumulative AGB + SNII + SNIa mass-return fraction of the cluster SSP at its age (same yield tables and Chabrier IMF as metal_return), updated at every active BH step. The lost mass is NOT deposited into the gas: the cluster mass is part of the star particles, whose own return (MetalReturnOn) already puts it there, so this is a mass update only and does not need MetalReturnOn. The lost mass leaves StarClusterMass (and P.Mass when StarClusterBHDyn=1) and is recorded in StarClusterTotalMassReturned; the cumulative loss through this channel alone is kept in the BH field SC_MlossStellar (code mass; in PART/PIG/SecPIG and the BH detail files). Requires StarClusterOn=1. Replaces the former StarClusterEvolution (stellar-evolution part).");
    param_declare_int(ps, "SCEvolutionRelaxation", OPTIONAL, 0, "Two-body relaxation mass loss of the star clusters attached to BH particles, applied at every active BH step. 0: off. 1: E-MOSAICS law (Kruijssen et al. 2011; Pfeffer et al. 2018 eq. 13), dM/dt = -M/t_dis with t_dis = t0_sun (M/Msun)^0.62 (T/T_sun)^-1/2, t0_sun = 21.3 Myr, T_sun = 7.01e2 Gyr^-2. 2: Gieles & Baumgardt (2008) / Alexander & Gieles (2012) law as in EMP-Pathfinder (Reina-Campos et al. 2022 eqs. 53-56), dM/dt = -xi M / t_rh with the Spitzer half-mass relaxation time (mean stellar mass 0.62 Msun as in AuriGLOBES, Coulomb log ln(0.11 N)) and xi = xi0 (1-P) + (3/5) zeta P (xi0 = 0.0142, zeta = 0.1), P set by r_h/r_t; r_h = (4/3) SC_Reff, the cluster's current effective radius (its initial radius SC_initReff -- the drawn R_eff, or the StarClusterReffRelation median at the cluster mass when the seeding path drew none -- unless StarClusterSizeEvolution=1 evolves it). Both use the E-MOSAICS tidal strength T = max(lambda) + Omega^2 of the BH tidal tensor (mode 2 through r_t = (G M / T)^1/3), so they require BlackholeTidalField=1: with SplitGravityTimestepsOn=0 the field is recomputed every BH gravity step, with SplitGravityTimestepsOn=1 the value from the last PM step is held. A BH without a field yet (seeded after the gravity of a PM step, it gets its first field at the next PM step when SplitGravityTimestepsOn=1) defers its relaxation, and the whole interval is applied with its first field. A cluster falling below 100 Msun is dissolved. The stripped mass leaves StarClusterMass (and P.Mass when StarClusterBHDyn=1) and is not returned to the gas; the cumulative loss through this channel alone is kept in the BH field SC_MlossRelax (code mass; in PART/PIG/SecPIG and the BH detail files). Requires StarClusterOn=1.");
    param_declare_int(ps, "SCRelaxCompactHeating", OPTIONAL, 0, "If 1, the GB08 two-body relaxation mass loss of the star clusters on BHs (SCEvolutionRelaxation=2) is enhanced by the heating from the compact remnants the clusters retain, following Guerra et al. (2026) eq. 13 after Gieles & Gnedin (2023): the rate is multiplied by a (m/m_i)^-2/3 with a = 1.5, m the current cluster mass and m_i its birth mass reduced by the stellar-evolution loss alone, so m/m_i is the fraction of the cluster's stars not yet lost to relaxation or tidal disruptions, from StarClusterMass, StarClusterTotalMassReturned and the four SC_Mloss records (capped at 1, e.g. after a restart from a snapshot without the records). A cluster that has lost nothing dynamically is boosted by a; the boost grows as it evaporates, and is held over each relaxation step. The size term of StarClusterSizeEvolution keeps the unboosted escape fraction xi (their eq. 22 as printed). The per-step relaxation log line reports the largest boost. Requires SCEvolutionRelaxation=2.");
    param_declare_int(ps, "StarClusterTDEtoBH", OPTIONAL, 0, "If 1, the BH particles grow by tidally disrupting the stars of the star cluster attached to them, at every active BH step. The disruption rate is Rizzuto et al. (2023) eq. 9, Ndot = 1.1 F f_b ln(0.22 M_BH/m_*) (M_BH/1e3 Msun) (rho/1e7 Msun pc^-3) (100 km/s/sigma)^3 per Myr with F = 0.8, f_b = 0.2, m_* = 1 Msun, where rho is the stellar density at the BH's influence radius (the sphere holding 2 M_BH of cluster stars) and sigma the 3-D velocity dispersion inside it, both from the Williams et al. (2026) cluster-with-BH structure built from StarClusterMass and the cluster's current SC_Reff: a Bahcall-Wolf cusp rho ~ r^-7/4 inside r_max = 1.4 R_eff with the isotropic Jeans dispersion sigma^2 = G (M(<r) + M_BH)/((1 + 7/4) r) (when 2 M_BH exceeds the cluster mass the whole cluster counts). Each disruption adds half the star's mass to the BH mass: the rates are stored in the BH fields NdotTDE and MdotTDE (per internal time unit; in PART/PIG/SecPIG and the BH detail files), and MdotTDE x dt is added to the BH mass on top of the gas accretion, which alone is limited by BlackHoleEddingtonFactor -- the sum can exceed that limit. The same mass is credited to Mtrack (it came from the cluster's stars, not from the gas, so the gas swallowing that makes Mtrack follow the BH mass is not triggered). The disrupted stars leave the cluster (StarClusterMass); with StarClusterBHDyn=1 the dynamical mass therefore loses the unbound half of each disrupted star. The cumulative cluster mass lost through this channel (the whole star per event, i.e. twice the BH gain) is kept in the BH field SC_MlossTDE (code mass; in PART/PIG/SecPIG and the BH detail files). Requires StarClusterOn=1 and BlackHoleOn=1; only BHs carrying a StarClusterMass and a radius are affected (see SCEvolutionStellar for which seeding paths attach a cluster).");
    param_declare_int(ps, "StarClusterEnhancedTDE4Merger", OPTIONAL, 0, "If 1, every BH-BH merger adds a burst of tidal disruptions to the remnant: the stars orbiting the LIGHTER BH of the pair are driven onto it by the companion through the eccentric Kozai-Lidov mechanism (Mockler et al. 2023). The number of disruptions is N_TDE = 0.25 x the number of cluster stars inside the hierarchical radius a_hier = 0.1 a_bin (1 - e^2)/e (their eq. 2) of a binary at a_bin = 0.5 r_h,1 with e = 0.5 (their fiducial setup), where r_h,1 is the sphere holding 2 m1 of the secondary's own cluster stars (at most the cluster's edge r_max = 1.4 R_eff) on the same rho ~ r^-7/4 cusp as StarClusterTDEtoBH; the 0.25 is the middle of their Table 1 range (lower limits 0.05-0.11, upper 0.27-0.45). With r_h,1 defined by the enclosed mass this gives N_TDE = 0.0196 m1/Msun whenever 2 m1 <= M_SC, and 0.0098 M_SC/Msun when the secondary outweighs half its cluster (0 without a cluster; the secondary's cluster is the swallowed BH's own when it is the lighter one, else the swallower's). The remnant gains 0.5 Msun per event, added at a constant rate over the 10 Myr after the merger (a new merger restarts the window for whatever is still pending, and a swallowed BH's unfinished burst passes to its remnant); the rate is the BH field MdotTDEMerger (code mass per internal time unit, like Mdot; in PART/PIG/SecPIG and the BH detail files), and the pending mass and window are MergerTDEMassLeft (code mass) and MergerTDETimeLeftMyr (Myr; both in the snapshots and read back on a restart, the former also in the detail files). The mass is added on top of the gas accretion, which alone is limited by BlackHoleEddingtonFactor, so the total growth rate Mdot + MdotTDE + MdotTDEMerger can exceed that limit; like the StarClusterTDEtoBH mass it is credited to Mtrack. The disrupted stars leave a star cluster too: while a burst drains, the whole stars of each step's share (twice the BH gain) are taken out of the cluster the remnant carries, i.e. the kept (heaviest) cluster of the merger, whichever of the two BHs brought it, with the same bookkeeping as the StarClusterTDEtoBH loss (StarClusterMass and StarClusterTotalMassReturned scaled down, the cluster removed below 100 Msun, the dynamical mass updated under StarClusterBHDyn=1). The cumulative cluster mass lost this way is the BH field SC_MlossTDEMerger (code mass; a property of the cluster like SC_MlossTDE: it follows the kept cluster at mergers and is reset with it; in PART/PIG/SecPIG, read back on a restart, and in the BH detail files). A remnant without a cluster still gains the burst mass. Independent of StarClusterTDEtoBH. Requires StarClusterOn=1 and BlackHoleOn=1.");
    param_declare_int(ps, "StarClusterSizeEvolution", OPTIONAL, 0, "If 1, evolve the effective radius SC_Reff of the star clusters attached to BH particles with their mass loss, following Guerra et al. (2026) sect. 2.2.3 (no tidal-shock term, which is not modelled). Their half-mass-radius prescription is applied to R_eff = (3/4) r_h: at fixed profile shape both change by the same fraction. Stellar-evolution term (only when SCEvolutionStellar=1): adiabatic expansion by the inverse ratio of the cluster mass after and before each stellar-evolution mass loss. Two-body-relaxation term (only when SCEvolutionRelaxation=2, GB08): dR_eff/R_eff = (2 - zeta/xi) dm_rlx/m (their eq. 22), expanding in isolation and contracting when tidally limited; the GB08 relaxation then uses the evolved radius. The E-MOSAICS relaxation (SCEvolutionRelaxation=1) has no size term. If neither mass-evolution model is on, size evolution is switched off with a warning. The start-up log states which terms are active. The cumulative ln(R_eff) change from each term is kept in the BH fields SC_dlnReffStellar and SC_dlnReffRelax (in PART/PIG/SecPIG and the BH detail files), so ln(SC_Reff/SC_initReff) is their sum.");

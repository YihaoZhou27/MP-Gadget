...
create_gadget_parameter_set()
{
    ...
    param_declare_int(ps, "SeedInSecFOFRandomStarParticle", OPTIONAL, 0, "Only used with SeedSecFOFcomSample=1 (and NOT SeedSecFOFcomSampleParticle, NOT SeedInSecFOFMultipleSeeds, NOT SeedSeedFOFMassiveBoundStar). If 1, the combined per-secFOF draw is not summed into one seed; instead every sampled cluster with mass >= MinMscForBHseed seeds its own BH (mass SeedBlackHoleMass*m_sc when BHseedMassScaleMsc=1, else SeedBlackHoleMass), each hosted on a randomly chosen distinct unseeded star of the group. If the eligible clusters outnumber the unseeded stars, a message is printed and the remaining clusters in that group are skipped. SCmasscapSecFOFstarmass has no effect in this mode.");

...
`SeedSecFOFcomSampleParticle = 1` and `SeedInSecFOFMultipleSeeds = 1` are still refused: the first redistributes cluster mass in `secondfof_seed`, after `fof_seed` has freed the mask, and the second places its extra hosts without a boundedness test. **Known asymmetry, unchanged by this commit:** a seeded group still has *every* seedable star flagged `Seeded = 1`, bound or not, so an unbound star is spent without having contributed to the budget. That matches what the single-seed path does (`secondfof_seed` marks every type-4 star of a seeded group), and changing it is a modelling decision for both paths rather than something the per-cluster mode should decide alone. Compiles and links cleanly with no warnings; still no end-to-end run.

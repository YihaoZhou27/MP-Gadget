...
## 2026-09-22 00:55 — `GWRecoilSCKick`: the cluster escape velocity is now that of the rho ~ r^-7/4 cusp, not of a Plummer sphere

The ejection test of a merger remnant from its star cluster (v_kick > v_esc) compared the GW kick with the central escape velocity of a Plummer sphere: 33.4 km/s (M_sc/1e5 Msun)^1/2 (r_h/pc)^-1/2 with r_h = 4/3 R_eff, i.e. v_esc^2 = 1.95 G M_sc/R_eff (the constant was never documented; it is sqrt(2 x 1.3 G 1e5 Msun/pc), the Plummer value with r_h = 1.3 a). At the user's request the cluster is now the Bahcall & Wolf (1976) cusp rho ~ r^-7/4 of the cluster+BH profile of Williams et al. (2026, arXiv:2603.26872), truncated at the cluster radius r_max: its central potential is -5 G M_sc/r_max and the projected half-mass radius of that profile is R_eff = 0.418 r_max (3-D half-mass radius 1.375 R_eff), so v_esc^2 = 10 G M_sc/r_max = 4.1756 G M_sc/R_eff, i.e. 42.4 km/s (M_sc/1e5 Msun)^1/2 (R_eff/pc)^-1/2 -- 1.47x the Plummer value at the same mass and size. As before, the BH's own potential is left out (it does not bind the BH) and the cluster mass, radius and their fallbacks are unchanged; the velocity is now formed from the code's G, Msun and pc instead of a hard-coded km/s constant. Parameter help text updated. Effect: fewer ejections -- at 1e5 Msun and R_eff = 1 pc, kicks between 29 and 42 km/s now keep the cluster. Every run made so far with `GWRecoilSCKick=1` (`SCevolution/paramfile_2TDE_recoil.gadget`) used the Plummer test. dev3 deliberately left unpatched (user's instruction); the running largerun and SCrun jobs use dev3's binary.

Compiles clean; `gadget/MP-Gadget` rebuilt at 00:54 (no queued or running job launches the dev2 binary; the pending COheating pair has `GWRecoilSCKick = 0`, so it is unaffected). Not yet run in a simulation. Not committed.

## 2026-09-22 00:47 — Fix (diagnostic only): the "largest compact-object boost" of the relaxation log line is now the largest boost applied

Review finding: the per-step log line of `SCRelaxCompactHeating` reported the boost evaluated at the start of each cluster's step, but the sub-cycled integrator recomputes the boost as the mass falls, so over a long interval (a deferred first step, or a strongly evaporating cluster) the larger values it actually applied never reached the log. `sc_relax_gb08_step` now hands back the largest boost of any sub-step, and the log line prints that ("largest compact-object boost applied"). No change to the evolution itself.

Verification: compiles clean, `gadget/MP-Gadget` rebuilt at 00:46 (the COheating pair had not been submitted yet, so it will launch this binary); heating checks extended by four for the new output (1 with heating off, a for a single sub-step at m = m_i, tracks the final mass over a long interval -- 32 applied against 1.5 at the start for a cluster that fell to 1% of m_i -- and 0 when no sub-step runs) and pass, fake-BH driver passes, t_rh unchanged. Not committed.

## 2026-09-22 00:22 — Change: mean stellar mass of the GB08 relaxation 0.42 -> 0.62 Msun (`SC_RLX_MSTAR`)

The Spitzer half-mass relaxation time and the escape-fraction interpolation of `SCEvolutionRelaxation=2` used a mean stellar mass of 0.42 Msun: the value EMP-Pathfinder adopts (Reina-Campos et al. 2022, eq. 54), from which the whole GB08 implementation of 2026-09-11 took its constants. Its source comment, however, called it the Chabrier IMF mean over 0.08-120 Msun, which it is not: integrating the code's own Chabrier IMF (`metal_return.c`, log10 form) gives 0.61 Msun over that range and 0.67 over 0.1-100 (Kroupa 0.58 / 0.64). AuriGLOBES (Guerra et al. 2026), the model the paper draft describes, uses 0.62 Msun (Chabrier, 0.1-100). The constant is now 0.62 Msun -- a choice to follow AuriGLOBES and our own IMF rather than EMP-Pathfinder, not a bug fix; the comment and the parameter help text are updated. Effect: t_rh is 27-30% shorter and the GB08 loss rate xi/t_rh 1.29-1.39x higher for 10^3-10^6 Msun clusters (e.g. 10^4 Msun, R_eff 1 pc: t_rh 96 -> 68 Myr). Every GB08 run before this date (`SCevolution/output_new`, `output_norecoil`, `output_2TDE`, `output_2TDE-recoil`) therefore has relaxation loss ~30% slower than AuriGLOBES (and equal to EMP-Pathfinder's) for identical clusters; the dev3 binary still has 0.42.

Verification: compiles clean; `gadget/MP-Gadget` rebuilt in place at 00:19 (the running largerun and SCrun jobs launch dev3); the heating checks (21) and the fake-BH bookkeeping driver (77 checks) pass against the rebuilt library, and it returns t_rh = 68.3 Myr for the example cluster, as expected for 0.62. First runs: `SCevolution/output_COheating` and its heating-off twin `output_noCOheating` (same binary, same PART_040 restart to z = 8), a clean A/B for the compact-object heating at the corrected mean mass; both are prepared and submitted by hand from a login node (sbatch is refused from the compute host used here). Not committed.

## 2026-09-21 23:40 — New parameter `SCRelaxCompactHeating`: compact-object heating boost of the GB08 two-body relaxation

`SCRelaxCompactHeating = 1` (default 0; requires `SCEvolutionRelaxation=2`) enhances the GB08 relaxation mass loss of the clusters on BHs for the heating by the compact remnants they retain, following Guerra et al. (2026, AuriGLOBES) eq. 13 after Gieles & Gnedin (2023): the rate is multiplied by a (m/m_i)^(-2/3) with a = 1.5, where m_i is the cluster's birth mass less its stellar-evolution loss, so m/m_i is the fraction of its stars not yet lost dynamically. That fraction is formed from the existing bookkeeping -- (`StarClusterMass` + `StarClusterTotalMassReturned`) over the birth mass (`StarClusterMass` + the four `SC_Mloss` records) -- so relaxation and both TDE channels raise the boost; it is capped at 1 (e.g. after a restart from a snapshot without the records), and held over each relaxation step while the sub-cycling recomputes the boost from the falling mass. A cluster that has lost nothing dynamically is boosted by a; the boost grows as it evaporates, so dissolution is shortened by more than a (a 1e3 Msun, R_eff = 1 pc cluster in T = 1e4 Gyr^-2 dissolves in 404 Myr instead of 1021 Myr). By choice, the size term of `StarClusterSizeEvolution` keeps the unboosted escape fraction in 2 - zeta/xi (their eq. 22 as printed), so the heated clusters expand faster in time. No new BH field or detail payload. The per-step relaxation log line reports the largest boost; the start-up message describes the term. Motivation: the paper draft attributes this enhancement to our relaxation model, which did not have it.

Verification (first in a scratchpad copy of the tree; `gadget/MP-Gadget` here was then rebuilt at 23:57, after confirming from the submitted batch scripts that the running largerun and SCrun jobs launch dev3's binary): compiles clean; with the switch off the relaxation helpers reproduce the unpatched library bit for bit on a 781-value grid; a fake-BH driver (77 checks) confirms m_i for clusters with no dynamical loss, half lost to relaxation, stellar loss only, all four channels, and the restart-like capped case, plus the dissolved, deferred (no field), swallowed and clusterless paths, and that heating off equals the plain step; `test_slotsmanager`, `test_metal_return`, `test_timebinmgr`, `test_forcetree` pass. First simulation: `codetest/LargeRunTest/SCevolution/output_COheating` (`paramfile_COheating.gadget`, `run_COheating.sh`), the twin of `output_2TDE` with the switch on: restarted from that run's PART_040 (a = 0.0653, the last snapshot before the first seed at a = 0.0687) to TimeMax = 0.111111111 (z = 8). Note that `output_2TDE` itself ran with dev3's binary from before 2026-09-19, so the pair also differs by the merger-burst cluster debit of that date. dev3 (source-identical for the touched files) was left unpatched. Not committed.

## 2026-09-19 17:50 — Assessed, not done (no code change): shared active-BH list for the cluster evolution routines; what the `SCRelax` timer really measures

Review suggestion: the three cluster routines (relaxation, stellar evolution, TDE growth) each scan all active particles, and relaxation and TDE growth add six MPI collectives per step; build the active-BH list once and share it. The description is right but the gain is negligible: in `SCevolution/output_2TDE` (segment R049, 125k steps, 59,700 s) the stellar routine costs 6.5 s and the TDE routine, which has the same structure as relaxation (one scan plus three collectives), 22 s, i.e. about 0.1% of the run for all three. Not worth changing.

The `SCRelax` timer nevertheless reads 3,880 s (6.5%; 6.9% in `output_norecoil`). That is not the routine: relaxation is the first of the three and opens with the first collective after the hydro step and the rank-local work that follows it, so the ranks' load imbalance is waited out there (per step the fastest rank spends ~0 in it, the slowest ~10 ms). In a run without cluster relaxation the same wait appears under `SPH/Metals` (7.5% in `PaperRuns_v3/SC-norecoil_newR/output_halo0_v1`, against 2.2-2.6% in the runs with relaxation). Read `SCRelax` in CPU-time tables as load imbalance, not as the cost of the relaxation model; removing or merging the collectives would only move the wait to the next synchronisation point. If an honest split is wanted, one timer call right after the opening collective of the first routine would book the wait under its own label.

## 2026-09-19 17:48 — Deferred (no code change): memory of the tidal-tensor buffer in the short-range gravity walk

Review suggestion: on PM steps the short-range gravity walk allocates a tidal-tensor buffer of 48 bytes for every particle, although with only the BH tidal field on just the BH entries are used, so BH-slot storage would remove almost all of it. Assessed and deferred. The premise does not occur in star-cluster runs: `StarClusterOn=1` requires `GasTidalField=1`, so the gas part of the buffer is really used and BH-slot storage alone would save nothing; only the dark-matter and star share (about half) could be recovered, by indexing the buffer by gas and BH slot. Sizes: 12.9 GB summed over the job for `SCevolution/output_2TDE` at z = 7 (about 1% of its memory), about 16 TB (4%, 7.8 GB per node) for a 2x5500^3 run on 2048 nodes, half of it recoverable. The buffer is transient (PM steps only, freed right after the walk). The change would re-index the reduce and post-processing of the most critical loop of the code for a 0.5-2% memory gain, so it is not worth doing now. Revisit only if the large run proves memory-bound on PM steps, and then with the gas + BH slot version.

Related, possibly more valuable, not measured: the result structure of the gravity walk carries the six tensor components for every exported particle of every type in every walk, including the sub-step walks where no tidal field is computed (about 88 instead of 40 bytes per exported result). Look here first if profiling of the large run shows the gravity walk to be communication-bound.

## 2026-09-19 17:31 — Fix: memory leak in the dying-mass helper for young stellar populations

`find_mass_bin_limits()` leaked its two GSL interpolation accelerators (64 bytes per call) whenever no star of the population had died yet, i.e. for ages below the lifetime of the 40 Msun star (4.3-5.5 Myr): that branch returned without freeing them. The bug is upstream's (since 2020) and hits both callers, the metal return of every young active star particle and, since `SCEvolutionStellar`, every active BH step of a young star cluster. Results were never affected and the amounts are small (at most ~28 MB summed over all ranks and restart segments of `SCevolution/output_2TDE` for the cluster path), and the GSL heap is separate from the code's own allocator. The branch now frees them.

Verification (in a copy of the tree; the binary here was not rebuilt, see the entry below): 5 million calls on a 1 Myr old population grow the heap by 320 MB with the old library and by nothing with the fix, both code paths return the same dying-mass limits as before, `test_metal_return` passes, and the cluster-debit driver still passes its 39 checks. Not committed.

## 2026-09-19 16:30 — `StarClusterEnhancedTDE4Merger`: the bursts now take their stars out of the remnant's cluster (`SC_MlossTDEMerger`)

Until now the merger-burst channel added mass to the BH but removed nothing from any star cluster. With `StarClusterEnhancedTDE4Merger=1` the disrupted stars now leave the cluster the remnant carries -- the kept (heaviest) cluster of the merger, whichever of the two BHs brought it -- while the burst drains: at every active step the whole stars of that step's share, i.e. twice the BH gain (1/f_acc), are removed with the same bookkeeping as the single-BH channel (cluster mass and returned mass scaled down, the cluster removed below 100 Msun, the dynamical mass updated under `StarClusterBHDyn=1`, the single-BH rate of the same step evaluated on the lighter cluster). The BH side is unchanged: the burst size is still set at the merger from the lighter BH's own cluster, and a remnant without a cluster still gains the mass (nothing is debited); a cluster smaller than the step's stars gives what it has. No new parameter: the debit belongs to the existing switch.

New BH field `SC_MlossTDEMerger`: the cumulative cluster mass lost this way (code mass), the fourth per-channel record next to `SC_MlossStellar` / `SC_MlossRelax` / `SC_MlossTDE`. It is a property of the cluster: zero at seeding, follows the kept cluster at mergers, reset when the cluster is dissolved or ejected; with a cluster on the BH, `StarClusterMass` plus the four records is the cluster's birth mass. Written to PART, PIG and SecPIG and read back on a restart (older snapshots count from the restart on); appended to the BH detail record (payload 532 -> 540 bytes). The per-step log line of the channel now also reports the stellar mass taken out of the clusters and the clusters eaten up. Parameter help text and start-up message describe the debit.

Post-processing: `code/bhdetails_raw2bf_v3.py` gets case 12 (`pm_sc_tidal_vort_screff_tde_channels_mtde_scloss`, auto-detected from the payload size); `script/bh_history.py` carries the new block into the traces when present.

Expected size of the effect, measured beforehand on `codetest/LargeRunTest/SCevolution/output_2TDE` at z = 7 (`script/bh_massbudget_tde.py`, `code_TDE/TDE_Mdot_channels.ipynb` Sec. 4c): 80 of 744 surviving clusters affected, by 0.004% in the median and 0.03% at most.

Verification: compiles clean (built and tested in a copy of the tree; the binary in this directory was NOT rebuilt because two queued jobs of `SCevolution` launch it). A fake-BH driver linked against the new `libgadget.a` passes 39 checks: the cluster loses exactly twice the BH gain per step and over a whole 1000 Msun burst, the record equals the loss and `StarClusterMass` + records stays the initial mass to round-off, the returned mass scales with the cluster, a BH without a cluster gains the burst with nothing debited, clusters of 150 and 250 Msun are dissolved (record and radii reset) while the BH still receives the whole burst, a swallowed BH and a run with the flag off are untouched, the single-BH channel alone reproduces its closed form after the refactoring of its cluster bookkeeping into a shared helper, and with both channels on the single-BH rate is evaluated on the debited cluster and the heavier BH. The detail record is 548 bytes (payload 540), matching converter case 12. Not yet run in a simulation. Not committed.

## 2026-09-17 19:35 — New parameter: TDE bursts at BH mergers (`StarClusterEnhancedTDE4Merger`)

New int parameter `StarClusterEnhancedTDE4Merger` (default 0, off; requires `StarClusterOn=1` and `BlackHoleOn=1`; independent of `StarClusterTDEtoBH`). With it on, every BH-BH merger adds a burst of tidal disruptions to the remnant following Mockler et al. (2023): the stars orbiting the lighter BH of the pair are driven onto it by the companion through the eccentric Kozai-Lidov mechanism, N_TDE = 0.25 x the cluster stars inside the hierarchical radius a_hier = 0.1 a_bin (1 - e^2)/e of a binary at a_bin = 0.5 r_h,1 with e = 0.5, where r_h,1 is the sphere holding 2 m1 of the secondary's own cluster stars (at most the cluster edge, 1.4 R_eff) on the same r^-7/4 cusp as the single-BH channel. Because r_h,1 is set by the enclosed mass the cluster cancels: N_TDE = 0.0196 m1/Msun whenever 2 m1 <= M_SC, and 0.0098 M_SC/Msun when the secondary outweighs half its cluster (0 without a cluster; R_eff never enters). The 0.25 is the middle of their Table 1 range (lower limits 0.05-0.11, upper limits 0.27-0.45; the post-processing study `code_TDE/TDE_rates.ipynb` and `code_TDE/TDE_MMBH_evolution.ipynb` used the same value). The constants are macros next to the `SC_TDE_*` ones (`SC_MTDE_EPS_DIS`, `SC_MTDE_ABIN_RH`, `SC_MTDE_EBIN`, `SC_MTDE_EPS_HIER`; the 10 Myr window is `SC_MTDE_TBIN_MYR` in the header), not parameters, so a different eps_dis needs a rebuild.

Mechanics: the burst is computed in the merger treewalk (`blackhole_feedback_ngbiter`), which now knows both masses and both clusters (the swallower's cluster mass and radius travel in the query), summed over the BHs swallowed in the step together with any burst mass those BHs had not yet added themselves, and put into the remnant's reservoir in `blackhole_feedback_postprocess`. Two new BH fields hold the reservoir, `MergerTDEMassLeft` (code mass) and `MergerTDETimeLeftMyr`, and every merger restarts the window to 10 Myr for whatever is pending. `starcluster_tde_growth()` (now taking both channel flags and called when either is on) drains the reservoir at a constant rate each active step -- the window closing inside a step finishes the burst -- adds the mass to the BH mass and to `Mtrack` (stellar debris, not gas, like the single channel), and stores the rate in the third new field `MdotTDEMerger`. The gas accretion alone stays limited by `BlackHoleEddingtonFactor`: the total growth rate `Mdot + MdotTDE + MdotTDEMerger` can exceed it. The reservoir is a property of the remnant BH, not of its cluster: it is not reset when the cluster is dissolved or ejected and it runs whether or not a cluster is still attached. All three fields are written to PART, PIG and SecPIG; the reservoir and window are read back on a restart so an open burst continues (older snapshots start with nothing pending), the rate is write-only. `MdotTDEMerger` and `MergerTDEMassLeft` are appended to the BH detail record (payload 516 -> 532 bytes).

Post-processing: `code/bhdetails_raw2bf_v3.py` gets case 11 (`pm_sc_tidal_vort_screff_tde_channels_mtde`, auto-detected from the payload size, so `run.sh` needs no change); `script/bh_history.py` carries the two blocks into the traces when present. Sort, dedupe and merger scripts are block-agnostic.

Verification: compiles clean; a fake-BH driver linked against `libgadget.a` checks the count against the closed forms (uncapped 0.0196 m1 and its independence of the cluster, capped 0.0098 M_SC and its independence of m1, the notebook's 55.7 events at m1 = 2.84e3 Msun) and 16 steps of ~1 Myr of the injection: a 1000 Msun burst is added in full at exactly 100 Msun/Myr with the last step carrying the remainder, mass and Mtrack gains equal the reservoir, sum(MdotTDEMerger dt) reproduces it to 1e-12, a second merger after 4 Myr adds its 500 Msun on top, the reservoir/window/rate are zero afterwards, the single channel runs unchanged next to it (a BH heavier by its burst disrupts 11% fewer cluster stars, as M^-0.7 says), swallowed BHs and a run with the flag off are untouched. The merger treewalk itself is not exercised by the driver. All unit tests that run pass (the four test binaries that fail to start are stale executables linked against the old libgsl.so.25). Not yet run in a simulation. Not committed.

## 2026-09-12 21:42 — Fix: `GWRecoilSCKick` escape velocity uses the evolved cluster mass and size, and the right mass unit

The ejection test of a merger remnant from its star cluster (v_kick > v_esc) had two problems. First, the cluster mass entering v_esc was converted to Msun without the 1/h of the code mass unit (1e10 Msun/h), so the mass used, and the one printed in the ejection log line, was h = 0.68 times the true mass; with v_esc scaling as M^0.38 through the size-mass relation this made escape ~14% too easy. Second, the cluster radius was not the cluster's own: it was recomputed from the Brown & Gnedin (2021) all-ages size-mass relation at the current mass, ignoring both the radius drawn at seeding (`SC_initReff`) and its evolution (`SC_Reff` under `StarClusterSizeEvolution`), so an expanded cluster still counted as compact. Now v_esc = 33.4 km/s (M/1e5 Msun)^1/2 (r_h/pc)^-1/2 is evaluated on the kept cluster's current `StarClusterMass` (h-corrected) and r_h = 4/3 `SC_Reff`, falling back to `SC_initReff` and then to the `StarClusterReffRelation` median only when the radius is not set yet after a restart from an older snapshot. The ejection log line now also prints the radius. The parameter help text describes the rule. Every run with `GWRecoilSCKick=1` made so far used the old, biased test.

Compiles clean. Not yet run in a simulation. Not committed.

## 2026-09-12 21:24 — Per-channel record of the star-cluster mass and size evolution (`SC_Mloss*`, `SC_dlnReff*`)

Until now a BH's star cluster carried only its current mass and radius (`StarClusterMass`, `SC_Reff` against `SC_initReff`), so the total change could be measured but not split by cause. Five new BH fields record what each channel did to the cluster, cumulatively since the cluster was attached: `SC_MlossStellar`, `SC_MlossRelax` and `SC_MlossTDE` are the cluster mass lost to stellar evolution (`SCEvolutionStellar`), to two-body relaxation (`SCEvolutionRelaxation` 1 or 2) and to tidal disruptions (`StarClusterTDEtoBH`, the whole star per event, i.e. twice what the BH gained), in code mass like `StarClusterMass`; `SC_dlnReffStellar` and `SC_dlnReffRelax` are the cumulative change of ln R_eff from the stellar-evolution and the GB08-relaxation term of `StarClusterSizeEvolution`. While a BH carries a cluster, `StarClusterMass` plus the three losses is the cluster's birth mass and ln(`SC_Reff`/`SC_initReff`) is the sum of the two size records. The records are cluster properties like the radii: they start at zero at seeding, follow the kept (heaviest) cluster at a BH merger, and are reset with the rest of the cluster state when the cluster is dissolved (by relaxation or eaten by TDEs) or ejected by a GW recoil, so the final breakdown of a lost cluster lives in the last BH detail record before that step. The fields are written to PART/PIG/SecPIG and read back on a restart (a restart from a snapshot without them counts from the restart on), and they are appended to the BH detail record (payload 476 -> 516 bytes). Parameter help texts of the four options mention their record.

Post-processing: `code/bhdetails_raw2bf_v3.py` gets case 10 (`pm_sc_tidal_vort_screff_tde_channels`) for the new record layout; `script/bh_history.py` carries the five blocks into the traces when present. The sort, dedupe and merger scripts are block-agnostic.

Verification: compiles clean; a driver with fake BHs run for 352 steps (~440 Myr, z = 11.5 -> 5.7) through the relaxation (GB08 with size), stellar evolution (with size) and TDE growth in the main-loop order confirms at every step the mass identity (to 4e-15), the radius identity (3e-15) and `SC_MlossTDE` = 2 x the BH mass gain (3e-17), for a cluster losing 30% to stellar evolution, 8% to relaxation and 5% to TDEs while expanding x2.5; a cluster without BH mass and tidal field records the stellar loss alone (equal to `StarClusterTotalMassReturned`, size record = ln(m0/m)); a cluster eaten by TDEs in one step and one dissolved by the relaxation come out with all records zero; clusterless and swallowed BHs are untouched. `test_slotsmanager`, `test_metal_return`, `test_forcetree`, `test_gravity`, `test_exchange`, `test_fof` pass. Not yet run in a simulation. Not committed.

## 2026-09-12 03:50 — New parameter: stellar-age limit on star-cluster BH seeding (`StarClusterSeedMaxAgeMyr`)

New parameter `StarClusterSeedMaxAgeMyr` (Myr, default 0). At 0 or below there is no limit and the behaviour is exactly as before. Above 0, only star particles younger than the limit at a seeding pass contribute to their group's star-cluster seeding budget, and only they may be chosen as the host star converted into the BH particle; the metallicity statistics, the sampled cluster population and the mass cutoff built from the same set follow. An over-age star is **not** consumed: it keeps its cluster mass, stays flagged unseeded, and is still a primary-linking particle, so the secondary-FOF groups themselves are unchanged and it would contribute again if the limit were lifted. Applies to every star-cluster seeding path (secondary-FOF combined and per-cluster, primary-FOF, and the per-star-particle path). The motivation is that the cluster formation efficiency is evaluated once, from the gas the star formed out of, so the limit bounds how stale that estimate can be when it is spent; `code_v3/SeededStar_AgeAtSeeding.ipynb` measures the ages that actually occur (median 2.2 Myr, 98.5% of the budget under 10 Myr, but 26% of the z > 12 budget would be lost at a 10 Myr limit), which argues for 30-50 Myr rather than anything near the seeding cadence.

Verification: compiles clean; the formation-time cutoff the limit is turned into round-trips to 1e-6 against the code's own age integral at z = 6, 7, 10 and 14.2 for limits of 1-300 Myr, including the case where the whole history is shorter than the limit; `test_fof` (7 tests) and the rest of the suite pass. Not yet run in a simulation. Not committed.

## 2026-09-12 03:35 — Fix: `StarClusterTDEtoBH` reports and stores the realised disruption rate

When a step's model rate would disrupt more stars than the cluster has, the mass taken was correctly capped at the cluster, but the per-step total in the log still added the model's number of events, and the stored `NdotTDE` was the uncapped model rate while `MdotTDE` was the capped one (a depletion test reported 51,186 events from a 1,000-star cluster that gave the BH 500 Msun). Both stored rates and the log total now describe what was applied: `NdotTDE` is the realised rate (the model rate unless the step ran out of stars, then the cluster's stars over the step) and `MdotTDE` = f_acc m_* `NdotTDE` exactly, so `Mass` grows by `MdotTDE` x dt to round-off. Driver checks added for the realised rate, the identity between the two fields and the event total; all pass. Compiles clean. Not committed.

## 2026-09-12 03:10 — `StarClusterTDEtoBH`: the TDE term of the cluster mass evolution is now unconditional

The removal of the disrupted stars from the cluster mass was behind a compile-time switch (`SC_TDE_DEPLETE`, default on). At the user's request the switch is gone: with `StarClusterTDEtoBH=1` every disruption always takes the whole star (m_*) out of `StarClusterMass` (half of it goes to the BH, the rest is unbound debris ejected far above the cluster's escape speed), rescaling `StarClusterTotalMassReturned` and dissolving a cluster eaten below 100 Msun as the relaxation does. The comparison in `code_v3/BHgrowth_TDE_Rizzuto23.ipynb` Sec. 8 puts this term at 0.1-4% of the cluster mass over 200 Myr, comparable to the GB08 relaxation loss and an order of magnitude below stellar evolution. No change in behaviour for the default build; start-up message updated. Compiles clean, fake-BH driver passes. Not committed.

## 2026-09-12 02:25 — Post-processing fix: BH histories discover detail blocks across restart segments

`script/bh_history.py` decided which optional detail blocks (`StarClusterMass`, `SC_Reff`, `NdotTDE`, `MdotTDE`, ...) go into a BH history by looking at a single, unsuffixed `bigfile-BlackholeDetails` directory, so a run that gains blocks at a restart (first segment written before a feature existed, later ones after) lost them from every history; in fact the unsuffixed directory does not exist for segmented runs, so no optional block was ever picked up. `trace_fields` now takes the union of the blocks present in any segment (sorted or raw), and both readers (`get_bhdata`, `bh_history_tree.get_bhdata_multi`) zero-fill a block for the segments that lack it. Reproduced with the TF_test segment plus a synthetic TDE-aware segment: the old records come back with the two TDE blocks at 0, the new ones with their values. Existing histories of segmented runs will gain the cluster blocks when regenerated.

## 2026-09-12 02:05 — Fix: TDE mass is credited to `Mtrack` as well as to the BH mass

`Mtrack` is the mass the BH particle has physically taken in, and `blackhole()` swallows gas particles with a probability set by `Mass - Mtrack` so that `Mtrack` follows the subgrid mass. The TDE growth of 01:30 raised `Mass` alone, so stellar-fed growth would have drawn the same mass out of the gas again (even with zero Bondi rate), and under `StarClusterBHDyn=1` the dynamical mass `Mtrack + StarClusterMass` lost the whole disrupted stellar mass, the half kept by the BH included. The retained mass is now added to `Mtrack` too (the same treatment `blackhole_make_one` gives a seed mass above its parent mass), so `Mass - Mtrack` is unchanged by a TDE step, no extra gas is swallowed, and with `StarClusterBHDyn=1` the dynamical mass loses only the unbound half of each disrupted star; `P.Mass` is rederived from `Mtrack` in the same step for the other modes. The `P.Mass` update for mode 1 now also runs when cluster depletion is compiled out. Fake-BH driver extended with these checks (Mtrack gain = mass gain, `Mass - Mtrack` unchanged, untouched for clusterless and swallowed BHs); all pass. Compiles clean. Not committed.

## 2026-09-12 01:30 — New feature: BH growth by tidal disruption of cluster stars (`StarClusterTDEtoBH`)

New parameter `StarClusterTDEtoBH` (0/1, needs `StarClusterOn=1` and `BlackHoleOn=1`). When on, every active BH that carries a star cluster disrupts the cluster's stars at the Rizzuto et al. (2023) eq. 9 rate, evaluated on the Williams et al. (2026) "cluster with black hole" structure (Bahcall-Wolf cusp, alpha = 7/4, inside r_max = 1.4 R_eff, the same profile family and normalisation as the CW seed model): the density at the BH's influence radius (the sphere holding 2 M_BH of cluster stars, capped at the cluster's edge) and the 3-D mean velocity dispersion inside it, from the BH mass, `StarClusterMass` and the cluster's current `SC_Reff`. Constants are fixed in the source (F = 0.8, f_b = 0.2, m_* = 1 Msun, half of each star to the BH). Two new BH fields, `NdotTDE` (disruption rate) and `MdotTDE` (BH growth rate from it), both per internal time unit like `Mdot`, are written to PART/PIG/SecPIG and to the BH detail record (payload 460 -> 476 bytes). Each active step `MdotTDE x dt` is added to the BH mass, on top of the gas accretion, which alone stays limited by `BlackHoleEddingtonFactor`; the disrupted stars leave the cluster (and P.Mass when `StarClusterBHDyn=1`), a cluster eaten below 100 Msun is dissolved like a relaxed one. The update runs in the main loop right after the cluster stellar evolution and before `blackhole()`, so the Bondi rate of the same step sees the heavier BH and the detail record carries that step's rates. The model, its conventions and the expected size of the effect are documented in `code_v3/BHgrowth_TDE_Rizzuto23.ipynb` (seeds: ~50 disruptions per Myr, ~0.3 Eddington; the channel matters for light seeds, +0.4% on the massive main branch).

Post-processing: `code/bhdetails_raw2bf_v3.py` gets case 9 (`pm_sc_tidal_vort_screff_tde`) for the new record layout; `script/bh_history.py` carries the two blocks into the traces when present. The sort, dedupe and merger scripts are block-agnostic.

Verification: compiles clean; the C rate function matches the notebook's on seven cases (including the whole-cluster cap and the zero guards) to 2e-13; a driver with fake BH particles confirms the per-step bookkeeping (mass gain = 0.5 m_* Ndot dt, MdotTDE dt = mass gain, NdotTDE in internal units, cluster depleted by m_* per disruption with `StarClusterTotalMassReturned` rescaled, a cluster eaten up is dissolved, clusterless and swallowed BHs untouched); `test_slotsmanager`, `test_metal_return`, `test_forcetree`, `test_gravity` pass. Not yet run in a simulation. Not committed.

## 2026-09-11 23:45 — Bug fix: stellar ages for the metal return were h times too small

`atime_to_myr` in `metal_return.c` integrates 1/(aH) in internal time units, where the time unit is `UnitTime_in_s / h` seconds, but converted the result to Myr with `UnitTime_in_s` alone. Every stellar age handed to the (physical) lifetime tables and to the SNIa delay-time distribution was therefore h = 0.6774 times too small — z = 11.17 -> 9 came out as 94.6 Myr instead of 139.6 Myr (astropy: 139.585 Myr) — so the whole mass and metal return of star particles was delayed by 1/h ~ 1.48. The conversion now divides by the Hubble parameter. Same expression, and the same fix, in dev, dev2, dev3; it is inherited from upstream MP-Gadget (master copy 3d59c2ee, 2026-08-18), not introduced here.

This changes the gas enrichment history of every future run: at a given time more of each SSP's mass and metals has been returned than before.

Star-cluster stellar evolution (`SCEvolutionStellar`) uses the same ages, so it is affected too: the TF_test seed cluster now loses 22.06% of its mass by z = 9 instead of 19.79% (driver against the rebuilt library; matches the 22.0% of `code_v3/ClusterMassLoss_GB08_SizeEvolution.ipynb`, which always used physical ages), and its R_eff grows by x1.283 instead of x1.247.

Compiles clean; `test_metal_return`, `test_cosmology`, `test_timefac`, `test_slotsmanager`, `test_gravity`, `test_forcetree` pass. Not committed.

## 2026-09-11 23:25 — Star-cluster stellar evolution no longer returns mass to the gas (mass-only model)

A cluster's mass is a budget (Gamma x birth mass) drawn from the star particles of its host group; those star particles keep their full mass and do their own stellar mass/metal return. Depositing the cluster's SSP ejecta into the gas as well counted that mass twice. `SCEvolutionStellar` is now a mass update only: at every active BH step the cluster mass becomes its birth mass times one minus the cumulative AGB + SNII + SNIa return fraction at the cluster's age (same yield tables and Chabrier IMF as `metal_return`), with the lost mass recorded in `StarClusterTotalMassReturned` and taken out of `P.Mass` when `StarClusterBHDyn = 1`; nothing is given to the gas and no metals are computed. The whole deposition machinery (SPH-volume walk, metal treewalk, gas-mass cap, rejection retry, the 1e-3 skip threshold and its bisection) is gone, and with it the two bugs those paths had. Because the update is cumulative rather than a sum of per-step windows, it no longer depends on the step pattern: BH steps are far shorter than the tolerance of the dying-mass root finding, which was what made the old per-step windows fragile. The size-evolution stellar term is unchanged (R_eff x m_old/m_new) but now follows this update.

Consequences for parameters: `SCEvolutionStellar` no longer needs `MetalReturnOn` (the start-up note about that is gone, and `StarClusterSizeEvolution`'s stellar term likewise only needs `SCEvolutionStellar`). The call now sits next to the relaxation in the main loop instead of inside the metal-return block. `metal_return` for star particles is untouched.

Verification with a driver calling the new routine on fake BHs (TF_test-like cluster, 8183 steps of ~0.017 Myr from z = 11.17 to z = 9): final cluster mass equals the one-shot birth mass x (1 - f) to 1e-16, birth mass is conserved exactly, R_eff/R_0 = m_0/m exactly, a relaxation-like rescaling of the cluster mid-run stays consistent, and clusterless, formation-time < 0, swallowed and non-BH particles are untouched. At 54 of 8183 steps the tabulated return fraction dips by up to 4e-4 (root-finding noise in the dying-mass limit); those steps simply apply no loss, and the error does not accumulate. Compiles clean; `test_metal_return`, `test_slotsmanager`, `test_gravity`, `test_forcetree` pass. Not committed.

Noted, not changed: `atime_to_myr` (upstream `metal_return.c`, used for both star and cluster ages) returns the interval in Myr x h, i.e. ages are 0.6774x too small with these parameters — z = 11.17 -> 9 comes out as 94.6 Myr instead of 139.6 Myr, so this cluster loses 19.8% instead of 22.1% by z = 9. Same expression in dev, dev2 and upstream master; a fix would change star-particle metal return in every run, so it awaits a decision.

## 2026-09-11 15:15 — Bug fix: star-cluster stellar return rejected by the gas is retried, not lost

In the star-cluster stellar-evolution walk a gas neighbour already at `MaxGasMass` (4x the mean gas mass) rejects its share, so the gas can take less than the intended return, or nothing. The cluster correctly lost only the deposited mass, but its last-enrichment time was still moved to the current age, so the rejected part of that window was never returned — and the adiabatic size expansion that goes with it (`StarClusterSizeEvolution`) never happened. (This is separate from the 1e-3 threshold fix of 12:35, which concerns returns before deposition.) Now the last-enrichment time advances only as far as the return was deposited: to the cluster's age when all of it went in, not at all when none did, and otherwise to the age whose return from the last enrichment equals the deposited mass (bisection on the code's own `mass_yield`). The undeposited rest stays in the cluster and is re-emitted at later steps. The metals deposited in a partial step carry the whole window's mean composition, a negligible mismatch for these short windows. When a step sees rejections, the log reports how many clusters were affected and the deposited fraction of the intended mass. Test with the code's `mass_yield` over 139 Myr of 0.017 Myr steps with heavy random rejection (30% of deposits refused, 30% partial): 11.6% of the initial mass returned with the old rule, 22.06% with the fix against 21.99% one-shot. `metal_return.c` treats star particles the same way (upstream behaviour) and is unchanged.

Compiles clean. Not committed.

## 2026-09-11 14:50 — Fix: per-step SC relaxation log line read unreduced totals on ranks > 0

The totals behind the per-step "SC relaxation: ..." message are reduced to rank 0 only, but every rank evaluated them in the print condition (undefined behaviour; `message(0, ...)` only suppresses the printing). The condition and message are now evaluated on rank 0 only. No change to the physics. Compiles clean. Not committed.

## 2026-09-11 14:45 — Relaxation holds the PHYSICAL tidal strength between field evaluations

The BH tidal eigenvalues are stored comoving and, with `SplitGravityTimestepsOn = 1`, refreshed only at PM steps. The star-cluster relaxation converted the cached values to physical with the *current* scale factor, so between evaluations the applied physical strength decayed as (a_PM/a)^3 — systematically weakening relaxation towards the end of every PM interval, unlike the validated PM-hold prescription (`code_v3/TidalField_PMcadence.ipynb`), which holds the physical strength. Each BH now records the scale factor at which its field was computed (`TidalFieldAtime`, stamped in the gravity tree walk; transient, not in snapshots) and the relaxation converts with that a, so the physical strength is held. The stamp (0 until the first field) also replaces the all-zero-eigenvalue test for the new-seed deferral. The stored eigenvalues and detail-file `TidalFieldStrength` stay comoving and unchanged; note that for runs with hierarchical gravity, post-processing should likewise convert a held record with the a of the BH's last PM-step record, not the record's own.

Size of the effect on the TF_test main-branch track (PM intervals dln a ~ 0.005): time-averaged T 0.76% low before the fix; mass lost by z = 9 changes by ~0.35% of the loss for E-MOSAICS and ~0.15% for GB08 (e.g. 38.92 -> 39.06% for a 1e4 Msun E-MOSAICS cluster). Small at these redshifts but systematic, and growing with the PM step.

Compiles clean; `test_gravity`, `test_slotsmanager`, `test_forcetree` pass. Not committed.

## 2026-09-11 14:05 — Two-body relaxation of new seeds before their first tidal field; GB08 sub-cycling

Seeding runs after the gravity of a PM step, and with `SplitGravityTimestepsOn = 1` the later sub-step trees hold only active particles, so a new BH has no tidal field until the next PM step (a PM interval, ~4 Myr in `TF_test`). Its relaxation used to run with the zeroed field meanwhile: no mass loss at all for E-MOSAICS, the isolated limit for GB08. Now a BH with no field yet is not relaxed; the elapsed time accumulates (new transient BH field `SC_RlxPendingMyr`, carried with the cluster at mergers, always 0 at a PM step and therefore not written to snapshots) and the whole interval is relaxed with the BH's first field. The per-step log reports how many clusters are waiting. Runs with `SplitGravityTimestepsOn = 0` are unaffected (the field is there at the first step after seeding).

Because such a deferred interval can span many relaxation times of a compact low-mass cluster, the GB08 update is now sub-cycled within a BH step so that no sub-step removes more than 0.2% of the cluster mass, with the size change (`StarClusterSizeEvolution`) fed back on t_rh between sub-steps. Ordinary BH steps still take a single sub-step, so their results are unchanged. Against an exact solution of the coupled mass/size equations, one 4.2 Myr step of a 3e3 Msun, 0.04 pc cluster is now within 0.05% in mass (single step: 3%). On the TF_test track the deferred, sub-cycled update reproduces the every-timestep reference to 2e-4 in M/M_0 after the first PM step for 1e4 and 3e3 Msun clusters (old behaviour: +0.002 and +0.005 for GB08, and no loss at all for E-MOSAICS).

Compiles clean; `test_gravity`, `test_slotsmanager`, `test_forcetree` pass. Not committed.

## 2026-09-11 12:35 — Bug fix: star-cluster stellar evolution discarded the small per-step mass returns

The stellar evolution of the star clusters on BH particles (`SCEvolutionStellar`, formerly `StarClusterEvolution`) applied almost none of its mass return. A step whose return is below 1e-3 of the cluster's initial mass is not applied, but the cluster path also moved the cluster's last-enrichment time forward, so that return was lost for good instead of being carried to the next step as `metal_return.c` does for star particles. BH steps are short (median ~0.02 Myr in `TF_test`), so a single step's return is typically ~1e-5 of the initial mass and practically every step was discarded: over 139 Myr at 0.017 Myr steps the code's own `mass_yield` gives 0 returned instead of 22% (and 0.6% instead of 22% on the recorded steps of the TF_test seed cluster, `code_v3/ClusterMassLoss_GB08_SizeEvolution.ipynb` Sec. 6). Now a small return accumulates until it reaches the threshold (same test: 22.01% in 212 chunks against 21.99% one-shot); only a return already capped at the maximum return fraction moves the time on unapplied, as in `metal_return.c`. The adiabatic size term of `StarClusterSizeEvolution` follows the applied returns, so it was affected the same way. Fixed in dev3 only; dev and dev2 keep the old behaviour.

Compiles clean. Not committed.

## 2026-09-11 04:05 — `StarClusterSizeEvolution`: evolving star-cluster radius; `SC_initReff` / `SC_Reff`; heaviest-cluster rule at BH mergers

New optional parameter `StarClusterSizeEvolution` (0/1, default 0) evolves the effective radius of the star cluster carried by each BH with its mass loss, following the half-mass-radius model of Guerra et al. (2026, AuriGLOBES) sect. 2.2.3 without its tidal-shock term (shocks are not modelled). The paper's prescription is for r_h; since both of its terms are fractional changes and r_h = (4/3) R_eff at fixed profile shape (the same relation the paper uses), it applies unchanged to our sampled R_eff. Two terms, each switched on only together with the mass-loss model it follows:
- stellar evolution (when `SCEvolutionStellar = 1` and `MetalReturnOn = 1`): adiabatic expansion by the inverse ratio of the cluster mass after and before each SSP mass return;
- two-body relaxation (when `SCEvolutionRelaxation = 2`, GB08, whose xi and zeta it needs): dR/R = (2 - zeta/xi) dm_rlx/m (their eq. 22), integrated over each step as (m_new/m_old)^(2 - zeta/xi) — expansion in isolation (exponent -5.0), contraction when tidally limited (+1/3). The GB08 mass loss now uses r_h = (4/3) × the current radius, so the two are coupled. The E-MOSAICS relaxation has no size term.
If `StarClusterSizeEvolution = 1` but neither model is active, size evolution is switched off with a warning; the start-up log always states which size terms are in use.

Every BH now carries two radii of its star cluster, in physical pc: `SC_initReff` (fixed initial value: the R_eff drawn at seeding, or the size–mass median at the cluster mass for seeding paths that draw none; this replaces this morning's `StarClusterReff`, which no run used) and `SC_Reff` (current value, starting at `SC_initReff`). Both are written to PART, PIG and SecPIG (`5/SC_initReff`, `5/SC_Reff`, read back on restart; snapshots without them load as 0 and are filled from `SC_initReff` or the median when first needed) and to the per-step BH detail record (payload 452 -> 460 bytes; `code/bhdetails_raw2bf_v3.py` case 8). Both are reset to 0 when the cluster is dissolved or ejected by a GW kick.

**Behaviour change for every run: BH-merger star clusters.** Previously the remnant's cluster was the sum of the merging clusters (mass-weighted metallicity, earliest formation time). Now the remnant keeps only the heaviest of the merging clusters — its own or a swallowed one, compared by cluster mass (ties to the larger BH ID), not inherited with the surviving BH ID — with that cluster's full state: mass, both radii, metallicity and metals, formation time and stellar-evolution bookkeeping. When several BHs are swallowed in one step the heaviest of all is kept. The lighter clusters are removed from the simulation: their mass is not added to the remnant, so with `StarClusterBHDyn = 1` it leaves `P.Mass`; the non-cluster mass of the swallowed BHs still enters `Mtrack` as before. Runs in which the BHs carry no cluster mass (e.g. `StarClusterBHDyn = 2` with secFOF seeding) are unaffected.

Verification: the size factor and the coupled GB08 mass + size stepping (0.05 Myr steps, 2 Gyr, isolated / weak / strong tidal field) follow a high-accuracy ODE solution of the paper's equations to <= 1.5e-4 (typically 1e-6); the new BH detail record is byte-identical to the converter's case-8 layout (record 468 B, checked field by field) and a synthetic case-8 file converts correctly. Compiles clean; `test_gravity`, `test_slotsmanager`, `test_forcetree` pass. Not yet exercised in a simulation run. Not committed.

## 2026-09-11 03:10 — `StarClusterEvolution` split into `SCEvolutionStellar` + `SCEvolutionRelaxation`; two-body relaxation of the BH star clusters

**Breaking parameter-file change:** `StarClusterEvolution` is removed (the parser now rejects it as unknown). Its stellar-evolution part is `SCEvolutionStellar` (0/1, default 0): the unchanged AGB + SNII + SNIa mass and metal return of the cluster SSP into the gas (still only active together with `MetalReturnOn = 1`). Replace `StarClusterEvolution = 1` by `SCEvolutionStellar = 1` to reproduce earlier runs.

New `SCEvolutionRelaxation` (0/1/2, default 0) switches on two-body relaxation mass loss of the star clusters carried by BH particles, applied over every active BH step:
- `1` — E-MOSAICS law (Kruijssen et al. 2011; Pfeffer et al. 2018 eq. 13): t_dis = 21.3 Myr (M/Msun)^0.62 (T/7.01e2 Gyr^-2)^-1/2, the same law as the `code_v3/ClusterMassLoss_*` / `TidalField_PMcadence` notebooks.
- `2` — Gieles & Baumgardt (2008) / Alexander & Gieles (2012) law with the constants adopted in EMP-Pathfinder (Reina-Campos et al. 2022 eqs. 53-56): dM/dt = -xi M / t_rh, Spitzer half-mass relaxation time with <m> = 0.42 Msun and ln(0.11 N), xi = 0.0142 (1-P) + 0.06 P, P from r_h/r_t with [r_h/r_t]_1 = 0.145, z = 1.61, m_1 = 1.5e4 <m>, x = 0.75. The cluster half-mass radius is r_h = (4/3) R_eff of the BH's birth cluster; clusters whose seeding path drew no radius use the median of `StarClusterReffRelation` at their mass.

Both are driven by the E-MOSAICS tidal strength T = max(lambda) + Omega^2 of the BH tidal tensor (mode 2 through the tidal radius r_t = (G M / T)^1/3; T <= 0 gives no loss in mode 1 and the isolated limit in mode 2), so they require `BlackholeTidalField = 1`. With `SplitGravityTimestepsOn = 0` the field is recomputed at every BH gravity step; `BlackholeTidalField` is now also allowed with `SplitGravityTimestepsOn = 1` (previously an error), in which case the value from the last PM step is held until the next one (the PM-hold prescription validated in `code_v3/TidalField_PMcadence.ipynb`). A cluster that falls below 100 Msun is dissolved (its cluster state is reset as for a GW-recoil ejection). The stripped stars leave `StarClusterMass` (and `P.Mass` when `StarClusterBHDyn = 1`) and are not returned to the gas; `StarClusterTotalMassReturned` is scaled down with the cluster, so the stellar-evolution return keeps acting on the initial mass of the stars still bound. Relaxation is applied before the stellar evolution of the same step.

Supporting changes: the BH slot now keeps the tidal-tensor eigenvalues (debug output block `5/BlackholeTidalFieldEigenvalues`) and the birth R_eff of the seed cluster (`5/BlackholeStarClusterReff`, read back on restart; only the per-cluster secFOF seeding path draws a radius, older snapshots load as 0 and fall back to the median). The BH detail record is unchanged. The start-up log describes the active relaxation model and tidal-field cadence, and each step reports the number of clusters evolved/dissolved and the stripped mass. Note that the per-cluster, combined-sample and compensating secFOF seeds attach a cluster payload only with `StarClusterBHDyn = 1`, as before, so both switches act on those seeds only in that mode (the start-up log says so otherwise).

Verification: the relaxation laws were checked against an independent implementation of the papers' equations — t_rh and xi agree to 1e-13, the E-MOSAICS update is exact (2e-11 against a high-accuracy ODE solution), and the GB08 step-by-step integration follows the ODE to ~1e-5 (2e-4 near destruction) at 0.05 Myr steps over 1 Gyr. Compiles clean; `test_gravity`, `test_slotsmanager` and `test_forcetree` pass. Not yet exercised in a simulation run. Not committed.

## 2026-09-09 00:20 — `StarClusterReffScatter`: user-set lognormal scatter of the star-cluster size–mass relation

New optional parameter `StarClusterReffScatter` (dex of log10(R_eff/pc), default -1). It controls the lognormal scatter drawn about the median size–mass relation selected by `StarClusterReffRelation`. A negative value (the default) keeps the scatter that belongs to the selected relation, 0.32 dex for `BG21` and 0.5 dex for `GBF`, so existing parameter files run unchanged; a value >= 0 replaces it for either relation (0 puts every cluster exactly on the median relation). Nothing else in the draw changes — same host-star-keyed Box-Muller Gaussian, same 0.1–100 pc clip — so only the width of the radius distribution moves. Like the relation, it is only used where cluster radii are actually sampled (`SecondFOFOn = 1` with `StarClusterFixReff = 0`); the start-up line now prints the scatter in use and whether it is the relation's own or the override, and a value that is set but inert is reported as ignored.

Motivation: decouple the width of the R_eff distribution from the choice of median relation, so the compact-radius tail that drives the M_seed/m_SC tail (`code_v3/Mseed_Msc_ZReff_clamp.ipynb`) can be varied on its own.

Compiles clean. Not committed.

## 2026-09-08 00:30 — `CWmodelMetallicityMin`: lower limit on the stars that set a sampled cluster's metallicity

New optional parameter `CWmodelMetallicityMin` (in Zsun, default 0 = off), used with `MbhMscRelationCWmodel = 1`. When > 0, unseeded stars of the host secFOF whose birth metallicity lies below the limit are masked out of the group's metallicity statistics that the CW-model cluster metallicity is drawn from, in every `CWmodelMetallicity` mode: the metal-mass ratio (`ave`), the log10(Z) mean and scatter (`lognormal`), the min/max (`uniform`) and the log10(Z) histogram (`starsample`). With 0.001 and `starsample`, for example, only stars with Z >= 1e-3 Zsun are sampled. A group with no star above the limit assigns exactly the limit to every one of its clusters. The mask is applied at the single accumulation point and mirrored in the `BHseedSecFOFbound` rebuild, with a separate star count so the equal-weight means keep the right denominator; the `MetUnseeded*` columns of the StarClusterDetails records therefore describe the masked star set. The choice of the host star converted into the BH particle is deliberately untouched: a star below the limit can still host the seed.

Motivation: the post-processing study of the RG1_v1 run showed that the seeds with M_seed/m_SC > 0.1 come almost entirely from clusters assigned Z < 1e-3 Zsun, below the Pop II / globular-cluster metallicity floor and far below the calibration range of the Vink wind law; the sweep in `code_v3/Mseed_Msc_ZReff_clamp.ipynb` showed a floor at 0.01 Zsun removes that tail. The start-up log states the active limit.

Compiles clean. Not committed.

## 2026-09-07 20:30 — `StarClusterReffRelation`: selectable size–mass relation (and scatter) for the sampled star clusters

New optional string parameter `StarClusterReffRelation`, used only with `SecondFOFOn = 1` (the secondary-FOF star-cluster seeding is the only place cluster radii are sampled). It selects the median size–mass relation from which every sampled cluster's effective radius is drawn, together with the lognormal scatter that belongs to it: `BG21` (the new default) is the Brown & Gnedin (2021) fit to the 1–10 Myr LEGUS clusters, R_eff = 2.365 pc (M_cl/1e4 Msun)^0.180 with 0.32 dex scatter; `GBF` is the relation the code used until now, 1.4 pc (M_cl/1e4 Msun)^0.25 with 0.5 dex. Both are projected effective radii; the CW seed-mass model keeps applying its own r_max = 1.4 R_eff, so no deprojection was added. The 0.1–100 pc clip, the ID-keyed reproducible draw and the `StarClusterFixReff` override are unchanged. An unrecognised value is an error when `SecondFOFOn = 1` and is ignored (BG21 kept, with a message) otherwise; the active relation is printed at start-up.

Motivation: the post-processing study `code_v3/Mseed_Msc_ZReff_clamp.ipynb` on `PaperRuns_v3/SC-norecoil/output_halo0_RG1_v1` showed that the M_seed/m_SC tail above 0.1 comes from the compact end of the 0.5 dex radius scatter; the BG21 relation with its measured 0.32 dex scatter removes most of it (seed count 8.7k -> 2.4k at fixed metallicity treatment). NOTE the default is the new relation, so runs meant to reproduce the earlier paper runs must set `StarClusterReffRelation = GBF`. The GW-recoil escape velocity in `blackhole.c` still hard-codes its own BG21 "all" relation and is not affected.

Compiles clean (dev3 built here for the first time; `Options.mk` copied from the dev2 tree). Not committed.

## 2026-08-21 (later) — `StarClusterCFEsigma`: cluster formation efficiency from the Kruijssen (2012) model with the measured gas velocity dispersion

New optional parameter `StarClusterCFEsigma` (0/1/2, default 0). 0 keeps the original prescription — the tabulated Kruijssen (2012) Gamma(P/k_B) at the thermodynamic pressure, i.e. sigma_loc = sqrt(P/rho) as in E-MOSAICS — unchanged at all three evaluation sites (cooling branch, star-formation branch, star birth), with the pressure formed exactly as before. 1 evaluates Kruijssen (2012) eq. 26 directly (new `get_cluster_formation_efficiency_k12()` in `starcluster.c`: lognormal density PDF, feedback-limited / incomplete / core efficiencies, 201-point quadrature in ln x, 0.02% accurate against a 1001-point reference and reproducing the table to 1-5% on EAGLE's EOS locus) from the particle's physical density and the 1-D dispersion of its star-forming gas neighbours `VDispGasSF`, measured in the density treewalk at the particle's own step (previous entry; requires `SCgasVDisp = 1`, which `StarClusterOn` already demands). A particle with fewer than 8 star-forming neighbours has `VDispGasSF = 0` and, by decision, gets the sigma = 0 limit of the model — a delta-function density PDF, Gamma = sSFR_ff t_sn / (eps_core t_ff) = 0.072 Myr / t_ff ~ 1.6e-3 (n_H/cm^-3)^0.5 — i.e. isolated star-forming particles form almost no bound clusters (2.6% of the z=6 star-forming particles, 0.13% of the SFR in SC-norecoil); no substitution by the all-gas dispersion. 2 is the same with the all-gas dispersion `VDispGasAll`. Model constants are those of the table (c_s = 0.3 km/s, eps_core = 0.5, sSFR_ff = 0.012, t_sn = 3 Myr, phi_fb = 0.16 cm^2 s^-3, t = 10 Myr, b = 0.5), evaluated in code units with the existing `t_sn_code` / `phi_fb_code` conventions plus new `cs_cold_code` / `t_inc_code`; the stored dispersions are internal velocities, divided by a at the call. The new star inherits the parent's value at birth (`ClusterFormationEfficiency`, `ClusterMass`), and the gas-side `ClusterFormationEfficiency` / `SumSFRdtCFE` follow the same rule.

Expected effect (post-processing of SC-norecoil at z=6, notebook Sections 7.3-7.4): star-forming-gas Gamma median 0.30 -> 0.15, SFR-weighted 0.87 -> 0.74; cluster-mass budgets x0.83 box-wide (FOF 0 x0.85) but x0.5 for ordinary star FOFs, i.e. a mass-dependent reduction. SCOPE: only Gamma changes; the M_cstar cloud-mass scale (Sigma_gas, t_fbg with sigma_loc = sqrt(P/rho), Toomre mass) is untouched and can get the same sigma later by a separate switch. `cooling_direct()` now takes GravInternal (for t_ff).

Verification (2026-08-21): new unit test `test_density_vdisp` compares `VDisp_gas_all` / `VDisp_gas_sf` / `VDisp_Ngas_sf` from the density walk against a brute-force O(N^2) evaluation (random positions, velocities and star-forming subset, periodic box): exact (N_sf mismatches 0, sigma to 1e-15; particles whose Hsml is clamped to MinGasHsml after their last walk are skipped, since their density/DivVel/dispersions belong to the pre-clamp Hsml -- pre-existing density-loop behaviour). First test run (`codetest/BH-L12.5-R1.0-Ng512-v6/output_newGamma`, restart PART_042 -> PART_043 at z=15, `script/check_cfesigma_run.py`): gas `ClusterFormationEfficiency` = eq. 26(rho, VDispGasSF/a) to 1.4e-4 including the sigma = 0 branch, new stars inherit it (median 0.15 vs 0.33 from the table), pre-restart stars bit-identical, `VDispGasSF = 0` wherever N_sf < 8. `VDispGasAll` agrees with the PM-step `VDispGas` to 6e-5 in quiet gas but up to a few per cent in star-forming regions: the density walk runs before the kick/star formation/winds of the step (predicted velocities with the previous accelerations), the PM walk after them -- expected, not a bug.

Compiles clean; `test_density` passes (4/4); standalone check of the eq. 26 routine against the Python reference. Not committed.

## 2026-08-21 — Per-step gas velocity dispersions from the density treewalk: `VDispGasAll`, `VDispGasSF`, `VDispGasNSF`

With `SCgasVDisp = 1` the SPH density treewalk now also measures, for every active gas particle at every one of its hydro steps, the 1-D number-weighted velocity dispersion of the gas within its smoothing length (`VDispGasAll`) and the same quantity restricted to the star-forming (Sfr > 0) neighbours (`VDispGasSF`), together with the number of star-forming neighbours (`VDispGasNSF`, self included). The estimator, neighbour set (all gas within Hsml, self included) and predicted velocities are those of the existing PM-step `VDispGas` walk, so at a PM step `VDispGasAll` should reproduce `VDispGas`; the difference is the cadence — fresh at the star-formation step instead of up to one PM step old — and the star-forming-only variant, motivated by the post-processing in `script/Gamma_EOS_compare_MPGadget_EAGLE.ipynb` (Sections 7.3-7.4: the star-forming-neighbour dispersion is ~half `VDispGas` and flat in pressure, and would change the cluster-mass budgets of ordinary star FOFs by ~x0.5). `VDispGasSF` is set to 0 when a particle has fewer than 8 star-forming neighbours (`VDISP_SF_MINNGB`); the count block says why. Values are in internal velocity units like `VDispGas` (divide by a for peculiar km/s).

Mechanics: the accumulators ride along the density walk (no new tree or treewalk; the result struct grows by 40 bytes), reduced into per-slot scratch arrays that are allocated only when `SCgasVDisp = 1`, and the fields are filled in the density postprocess, so the final hsml iteration wins exactly as for `DivVel`/`CurlVel`. The three blocks are registered with the other VDisp blocks in the debug set (written to PART/PIG/SecPIG when `OutputDebugFields = 1`). MEASUREMENT ONLY: nothing reads the new fields yet — the CFE still uses the thermal pressure and `phi_P` still uses the PM-step `VDispGas`. With `SCgasVDisp = 0` the walk is unchanged apart from the larger result struct. The `test_density` fixture now runs with `SCgasVDisp = 1` so the accumulator path (allocation, reduce, postprocess) is exercised by the regression tests. Suggested functional check on the next short test run: at a PM-step snapshot `VDispGasAll` must equal `VDispGas`, and `VDispGasSF` should match the KD-tree post-processing of notebook Section 7.3.

Compiles clean; `test_density` passes (3/3). Not committed.
